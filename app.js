/* Liquorne - app.js (v3.7 corrective)
   Fix: remove mixed legacy code, restore menu/back, fix syntax errors.
*/
window.__SPIRITS__ = [
  {
    id: "s1",
    name: "Highland 12",
    brand: "Liquorne Distilling",
    type: "Whisky",
    country: "Écosse",
    abv: 40,
    notes: ["miel", "vanille", "fruits secs"],
    imageDataUrl: "",
  },
  {
    id: "s2",
  ...
  },
  {
    ...
    country: "Mexique",
    abv: 40,
    notes: ["agave", "caramel", "boisé"],
    imageDataUrl: "",
  },
  {
    id: "s5",
    name: "Cognac VSOP Réserve",
    brand: "Maison Lune",
    type: "Cognac",
    country: "France",
    abv: 40,
    notes: ["abricot", "chêne", "épices"],
    imageDataUrl: "",
  },
  {
    id: "s6",
    name: "Vodka Pure Grain",
    brand: "Nordik",
    type: "Vodka",
    country: "Pologne",
    abv: 40,
    notes: ["céréales", "poivre blanc", "net"],
    imageDataUrl: "",
  },
];
...
(() => {
  try {
    ...
    function seedSpirits() {
      return (window.__SPIRITS__ || []).slice();
    }
    ...
    function defaultUsers() {
      return [{ id: "u-demo", username: DEMO_USER, email: "demo@liquorne.local", password: DEMO_PASS, createdAtIso: new Date().toISOString() }];
    }
    ...
    function goBack() {
      const prev = state.route?.prevTab ? state.route.prevTab : "home";
    ...
    function spiritCardHtml(s) {
      ...
      const badges = [st.owned ? '<span class="badge">Possédé</span>' : "", st.tasted ? '<span class="badge">Goûté</span>' : "", st.wishlist ? '<span class="badge">Wishlist</span>' : ""]
        .filter(Boolean)
        .join("");
    ...
    function addSpiritView() {
      ...
      const countries = uniqueSorted(
        state.spirits
          .map((s) => s.country)
          .filter(Boolean)
          .concat(["France", "Écosse", "Irlande", "USA", "Mexique", "Japon", "Martinique", "Guadeloupe", "Italie", "Espagne", "Canada", "Autre"])
      );
            ...
            <div class="card">
              ...
              <div style="display:flex; gap:14px; flex-wrap:wrap; align-items:center">
                ...
                <div style="flex:1; min-width:240px; display:flex; flex-direction:column; gap:10px">
                  ...
                  <div class="small">Prototype : ajout sans recadrage avancé (pour l’instant).</div>
                </div>
              </div>
            </div>
    ...
    function render() {
      ...
      // Auth redirect
      if (!isLoggedIn()) {
        if (state.route.name !== "signup") state.route = { name: "login", spiritId: null, prevTab: "home" };
      ...
      if (state.route.name === "signup") {
        ...
        document.getElementById("doSignup").addEventListener("click", () => {
          ...
          if (username.length < 3) {
            state.signupError = "Username : 3 caractères minimum.";
            render();
            return;
          }
          if (state.users.some((u) => (u.username || "").toLowerCase() === unameLow)) {
            state.signupError = "Ce username est déjà utilisé.";
            render();
            return;
          }
          if (!validateEmail(email)) {
            state.signupError = "Email invalide.";
            render();
            return;
          }
          if (email && state.users.some((u) => (u.email || "").toLowerCase() === email.toLowerCase())) {
            state.signupError = "Cet email est déjà utilisé.";
            render();
            return;
          }
          if (p1.length < 8) {
            state.signupError = "Mot de passe : 8 caractères minimum.";
            render();
            return;
          }
          if (p1 !== p2) {
            state.signupError = "Les mots de passe ne correspondent pas.";
            render();
            return;
          }
      ...
      if (state.route.name === "home") {
        ...
        abvMin.addEventListener("input", (e) => {
          state.filters.abvMin = e.target.value;
          rerender();
        });
        abvMax.addEventListener("input", (e) => {
          state.filters.abvMax = e.target.value;
          rerender();
        });
        sort.addEventListener("change", (e) => {
          sortHome = e.target.value;
          rerender();
        });

        document.getElementById("fabAdd")?.addEventListener("click", () => navTo({ name: "add", spiritId: null, prevTab: "home" }));
      ...
      if (state.route.name === "cellar") {
        ...
        document.getElementById("fabAdd")?.addEventListener("click", () => navTo({ name: "add", spiritId: null, prevTab: "cellar" }));
      ...
      if (state.route.name === "add") {
        ...
        document.getElementById("fileInputAdd")?.addEventListener("change", async (e) => {
          ...
          r.onload = () => {
            state.addDraft.imageDataUrl = r.result;
            render();
          };
          r.readAsDataURL(f);
        });
        ...
        document.getElementById("saveSpirit")?.addEventListener("click", () => {
          ...
          if (!state.addDraft.name.trim()) {
            alert("Nom requis (ex: Talisker 10).");
            return;
          }
    ...
    if (a) {
      a.innerHTML = `
        <div style="padding:16px;font-family:system-ui;color:#fff;background:#071022;min-height:100vh">
          ...
          <div style="opacity:.8;margin-top:12px">Astuce: ouvre l'URL avec <b>?nocache=1</b>.</div>
        </div>`;
    }
    try {
      console.error(e);
