...
/* ==========================================================
   Liquorne — Prototype PWA (web)
   Version: v3.8.0 (propre)
   - Navigation stable (home/cave/détail/ajouter/modifier)
   - Menu hamburger + Déconnexion
   - CRUD spiritueux (ajout + modification + suppression)
   - Photo via galerie (dataURL) + suppression
   - Données locales (localStorage) — prototype
   ========================================================== */

window.__SPIRITS__ = window.__SPIRITS__ || [
  {"id":"s1","name":"Highland 12","brand":"Liquorne Distilling","type":"Whisky","country":"Écosse","abv":40,"notes":["miel","vanille","fruits secs"],"imageDataUrl":""},
  {"id":"s2","name":"Caña Dorada","brand":"Casa del Sol","type":"Rhum","country":"Martinique","abv":50,"notes":["canne","agrumes","poivre"],"imageDataUrl":""},
  {"id":"s3","name":"Juniper No. 3","brand":"North Bay","type":"Gin","country":"France","abv":43,"notes":["genièvre","citron","herbes"],"imageDataUrl":""},
  {"id":"s4","name":"Reposado Azul","brand":"Agave Real","type":"Tequila","country":"Mexique","abv":40,"notes":["agave","caramel","boisé"],"imageDataUrl":""},
  {"id":"s5","name":"Cognac VSOP Réserve","brand":"Maison Lune","type":"Cognac","country":"France","abv":40,"notes":["abricot","chêne","épices"],"imageDataUrl":""},
  {"id":"s6","name":"Vodka Pure Grain","brand":"Nordik","type":"Vodka","country":"Pologne","abv":40,"notes":["céréales","poivre blanc","net"],"imageDataUrl":""}
];
...
(() => {
  ...
  const K = {
    reviews:  "liquorne_reviews_v3_8",
    cellar:   "liquorne_cellar_v3_8",
    session:  "liquorne_session_v3_8",
    spirits:  "liquorne_spirits_v3_8",
    users:    "liquorne_users_v3_8",
  };
  ...
  let state = {
    route: { name: "login", spiritId: null, prevTab: "home" }, // name: login|signup|home|cellar|detail|add|edit|addReview
    q: "",
    filters: { type:"", country:"", abvMin:"", abvMax:"" },
    sortHome: "rating_desc",
    sortCellar: "rating_desc",
    ...
    editDraft: null, // {id, imageDataUrl, name, brand, type, country, abv, notes}
  };
  ...
  function goBack(){
    ...
    navTo({ name: prev, spiritId: null, prevTab: prev });
  }

  function logout(){
    state.session = null;
    persist();
    navTo({ name:"login", spiritId:null, prevTab:"home" });
  }
      ...
      <div class="sideMenu" id="sideMenu" style="display:none" data-open="0" role="dialog" aria-modal="true">
        <div class="sideHeader">
          <div>
            ...
            <div class="sideSub">Compte local (prototype)</div>
          </div>
          <button class="sideClose" id="menuCloseBtn" onclick="closeMenu()" aria-label="Fermer">✕</button>
        </div>


        <a href="#" class="sideItem premium" onclick="closeMenu(); alert('Mon compte (à venir)'); return false;">
          <span class="sideLabel">Mon compte</span><span class="sideChevron">›</span>
        </a>
        <a href="#" class="sideItem premium" onclick="closeMenu(); alert('Paramètres (démo)'); return false;">
          <span class="sideLabel">Paramètres</span><span class="sideChevron">›</span>
        </a>
        <a href="#" class="sideItem premium" onclick="closeMenu(); alert('Aide (démo)'); return false;">
          <span class="sideLabel">Aide</span><span class="sideChevron">›</span>
        </a>

        <div class="sideDivider"></div>

        <a href="#" class="sideItem premium danger" onclick="closeMenu(); logout(); return false;">
          <span class="sideLabel">Déconnexion</span><span class="sideChevron">›</span>
        </a>

        <div class="sideFooter">
          <span class="sideFooterBrand">Liquorne</span>
          <span class="sideFooterVersion">v${APP_VERSION}</span>
        </div>
      ...
      <div class="topbar">
        <div class="topbarInner">
          ...
          <div class="brand brandCenter">
            ...
            <div class="brandText">Liquorne</div><div class="versionChip">v${APP_VERSION}</div>
          </div>

          <div class="actions actionsCenter">
            <button class="pill ${active==="home" ? "" : "ghost"}" onclick="navTo({name:'home',spiritId:null,prevTab:'home'})">Explorer</button>
            <button class="pill ${active==="cellar" ? "" : "ghost"}" onclick="navTo({name:'cellar',spiritId:null,prevTab:'cellar'})">Ma cave</button>
          </div>
        </div>
      </div>
      ${menuHtml()}
    ...
    return `
      <div class="topbar topbarBack">
        <div class="topbarInner">
          ...
          <div class="brand brandCenter">
            <img class="brandLogoSmall" src="./assets/logo-diamond-256-v34.png" alt="Liquorne"/>
            <div class="brandText">${esc(title || "Liquorne")}</div><div class="versionChip">v${APP_VERSION}</div>
          </div>
  ...
  function spiritCardHtml(s, prevTab){
    ...
    return `
      <div class="card item" style="display:flex; gap:12px; align-items:center">
        <div role="button" tabindex="0" data-open="${esc(s.id)}" style="display:flex; gap:12px; align-items:center; flex:1; min-width:0">
          ${thumbHtml(s)}
          <div style="min-width:0">
            ...
            <div class="tags">${notes.map(n => `<div class="chip">${esc(n)}</div>`).join("")}</div>
          </div>
        </div>
        <div class="scoreBox">
  ...
  function filtersHtml(sortKey){
    ...
    return `
      <div class="card">
        ...
        <div class="row">
          <div class="small">Prototype local • Photos et notes sur cet appareil</div>
            ...
            <div class="authB">
              ...
              <div class="authCardB">
                ...
                <div class="small" style="opacity:.85; margin-top:2px">
                  Prototype : données locales sur ce téléphone.
                </div>
              </div>
            </div>
          </div>
  ...
  function homeView(){
    ...
    const sorted = sortList(filtered, state.sortHome);
    const list = sorted.map(s => spiritCardHtml(s, "home")).join("");
  ...
  function cellarView(){
    ...
    function section(title, items){
      const sorted = sortList(items, state.sortCellar);
      return `
        <div class="card">
          <div class="row">
            <div>
          ...
          <div class="divider"></div>
          <div class="list">
            ${sorted.length ? sorted.map(s => spiritCardHtml(s, "cellar")).join("") : `<div class="small">Rien pour le moment.</div>`}
          </div>
        </div>
      `;
    }
    ...
    return `
      <div class="page light">
        ...
        <div class="container">
          <div class="card">
            <div class="row">
              ...
              <div style="display:flex; gap:10px; flex-wrap:wrap">
                <select id="sortCellar" style="max-width:260px">
                  <option value="rating_desc" ${state.sortCellar==="rating_desc" ? "selected" : ""}>Trier : note ↓</option>
                  <option value="name_asc" ${state.sortCellar==="name_asc" ? "selected" : ""}>Trier : nom A→Z</option>
                  <option value="abv_desc" ${state.sortCellar==="abv_desc" ? "selected" : ""}>Trier : ABV ↓</option>
                  <option value="abv_asc" ${state.sortCellar==="abv_asc" ? "selected" : ""}>Trier : ABV ↑</option>
                </select>
          ...
          ${section("✅ Possédés", owned)}
          ${section("🍸 Goûtés", tasted)}
          ${section("⭐ Wishlist", wish)}
        </div>
        ${fabHtml("cellar")}
      </div>
    `;
  ...
  function detailView(spirit){
    ...
    const avg = rs.length ? (rs.reduce((a,r)=>a + Number(r.rating||0),0)/rs.length) : null;
    const st = getStatus(spirit.id);

    const image = spirit.imageDataUrl && spirit.imageDataUrl.startsWith("data:image/")
      ? `<img src="${spirit.imageDataUrl}" alt="photo" style="width:132px;height:132px;object-fit:cover;border-radius:24px;border:1px solid rgba(255,255,255,.12)" />`
      : `<div class="thumb" style="width:132px;height:132px;border-radius:24px"><div class="ph">PHOTO</div></div>`;
    ...
    return `
      <div class="page light">
          ...
          <div class="card" style="display:flex; gap:14px; align-items:flex-start; flex-wrap:wrap">
            ...
            <div style="flex:1; min-width:240px; display:flex; flex-direction:column; gap:6px">
              ...
              <div class="row" style="justify-content:flex-start; gap:10px">
                <button class="btn" id="editSpirit">✏️ Modifier</button>
                <button class="btnGhost" id="addReview">+ Avis</button>
              </div>
        ...
        ${fabHtml(state.route.prevTab)}
      </div>
    `;
  }

  function addSpiritFormView(mode){
    // mode: "add" | "edit"
    const isEdit = mode === "edit";
    const d = isEdit ? state.editDraft : state.addDraft;
    ...
    const hasPhoto = d?.imageDataUrl && d.imageDataUrl.startsWith("data:image/");
    const img = hasPhoto
      ? `<img src="${d.imageDataUrl}" alt="photo" style="width:120px;height:120px;object-fit:cover;border-radius:24px;border:1px solid rgba(255,255,255,.12)" />`
      : `<div class="thumb" style="width:120px;height:120px;border-radius:24px"><div class="ph">PHOTO</div></div>`;

    const title = isEdit ? "Modifier un spiritueux" : "Ajouter un spiritueux";
    const top = isEdit ? topbarBack("Modifier") : topbarBack("Ajouter");
    const primary = isEdit ? "Enregistrer" : "Ajouter";
    const fileId = isEdit ? "fileInputEdit" : "fileInputAdd";
    const removeId = isEdit ? "removePhotoEdit" : "removeDraft";
    const saveId = isEdit ? "saveEdit" : "saveSpirit";
      ...
      <div class="page light">
        ...
        <div class="container" style="max-width:820px">
          <div class="card">
            <div class="row">
              <div>
                ...
                <div class="small">Import photo via galerie (prototype)</div>
              </div>
              ${isEdit ? `<button class="btnDanger" id="deleteSpirit">Supprimer</button>` : `<button class="btnGhost" id="clearDraft">Réinitialiser</button>`}
            </div>
              ...
              <div style="flex:1; min-width:240px; display:flex; flex-direction:column; gap:10px">
                <div class="row" style="justify-content:flex-start">
                  <label class="btn" style="cursor:pointer">
                    📁 ${isEdit ? "Changer la photo" : "Importer une photo"}
                    <input id="${fileId}" type="file" accept="image/*" style="display:none" />
                  </label>
                  <button class="btnDanger" id="${removeId}" ${hasPhoto ? "" : "disabled"}>Retirer</button>
                </div>
                <div class="small">Photos et données stockées localement (non sécurisé).</div>
              </div>
            </div>
          </div>
          ...
          <div class="card">
            ...
            <input class="input" id="name" placeholder="Ex: Talisker 10" value="${esc(d?.name || "")}" />

            <div style="height:10px"></div>
            <div class="small">Marque / Distillerie</div>
            <input class="input" id="brand" placeholder="Ex: Talisker" value="${esc(d?.brand || "")}" />
            ...
            <div class="grid" style="grid-template-columns:1fr 1fr 1fr;">
              <div>
                ...
                <select id="type">
                  ${types.map(t => `<option value="${esc(t)}" ${(d?.type || "Whisky")===t ? "selected" : ""}>${esc(t)}</option>`).join("")}
                </select>
              </div>
              <div>
                <div class="small">Pays</div>
                <select id="country">
                  ${countries.map(c => `<option value="${esc(c)}" ${(d?.country || "France")===c ? "selected" : ""}>${esc(c)}</option>`).join("")}
                </select>
              </div>
              <div>
                <div class="small">ABV %</div>
                <input class="input" id="abv" inputmode="decimal" placeholder="40" value="${esc(d?.abv ?? "40")}" />
              </div>
            </div>

            <div style="height:10px"></div>
            <div class="small">Notes (séparées par des virgules)</div>
            <input class="input" id="notes" placeholder="vanille, miel, tourbe" value="${esc(d?.notes || "")}" />

            <div style="height:14px"></div>
            <div class="row">
              <button class="btn" id="${saveId}">${primary}</button>
              <button class="btnGhost" id="cancelForm">Annuler</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="page light">
        ...
        <div class="container" style="max-width:720px">
          ...
          <div class="card">
            ...
            <button class="btn" id="saveReview">Enregistrer</button>
          </div>
        </div>
      </div>
    `;
  ...
  // ------- Bindings helpers -------
  function bindCards(prevTab){
    app.querySelectorAll("[data-open]").forEach(el => {
      el.addEventListener("click", () => {
        const id = el.getAttribute("data-open");
        navTo({ name:"detail", spiritId:id, prevTab });
      });
      el.addEventListener("keydown", (ev) => { if (ev.key === "Enter") el.click(); });
    });
  }

  function bindFab(prevTab){
    const b = document.getElementById("fabAdd");
    if (!b) return;
    b.addEventListener("click", () => {
      state.addDraft = { imageDataUrl:"", name:"", brand:"", type:"Whisky", country:"France", abv:"40", notes:"" };
      navTo({ name:"add", spiritId:null, prevTab });
    });
  }
  ...
  function render(){
    ...
    if (state.route.name === "login"){
      ...
      if (login) login.value = DEMO_USER;
    ...
    if (state.route.name === "home"){
      ...
      document.getElementById("type")?.addEventListener("change", (e) => { state.filters.type = e.target.value; render(); });
      document.getElementById("country")?.addEventListener("change", (e) => { state.filters.country = e.target.value; render(); });
      document.getElementById("abvMin")?.addEventListener("input", (e) => { state.filters.abvMin = e.target.value; render(); });
      document.getElementById("abvMax")?.addEventListener("input", (e) => { state.filters.abvMax = e.target.value; render(); });
      document.getElementById("sort")?.addEventListener("change", (e) => { state.sortHome = e.target.value; render(); });
    ...
    if (state.route.name === "cellar"){
      ...
      document.getElementById("sortCellar")?.addEventListener("change", (e) => { state.sortCellar = e.target.value; render(); });
    ...
    if (state.route.name === "detail"){
      ...
      document.getElementById("editSpirit")?.addEventListener("click", () => {
        // build editDraft
        state.editDraft = {
          id: spirit.id,
          imageDataUrl: spirit.imageDataUrl || "",
          name: spirit.name || "",
          brand: spirit.brand || "",
          type: spirit.type || "Whisky",
          country: spirit.country || "France",
          abv: String(spirit.abv ?? "40"),
          notes: (spirit.notes || []).join(", "),
        };
        navTo({ name:"edit", spiritId: spirit.id, prevTab: state.route.prevTab || "home" });
      });

      document.getElementById("addReview")?.addEventListener("click", () => {
        navTo({ name:"addReview", spiritId: spirit.id, prevTab: state.route.prevTab || "home" });
      });
      ...
      bindFab(state.route.prevTab || "home");
      return;
    }
    ...
    if (state.route.name === "add"){
      if (!ensureAuth()) return;
      app.innerHTML = addSpiritFormView("add");

      document.getElementById("cancelForm")?.addEventListener("click", () => goBack());
      ...
      document.getElementById("removeDraft")?.addEventListener("click", () => {
        state.addDraft.imageDataUrl = "";
        render();
      });
      ...
      document.getElementById("saveSpirit")?.addEventListener("click", () => {
        ...
        if (!name){ alert("Nom requis (ex: Talisker 10)."); return; }

        const brand = (document.getElementById("brand")?.value || "").trim() || "—";
        const type = document.getElementById("type")?.value || "Autre";
        const country = document.getElementById("country")?.value || "—";
        const abv = Number(document.getElementById("abv")?.value || 0) || 0;
        const notes = (document.getElementById("notes")?.value || "")
          .split(",").map(x => x.trim()).filter(Boolean).slice(0, 8);

        const id = "s-" + cryptoRandomId();
        const newSpirit = { id, name, brand, type, country, abv, notes, imageDataUrl: state.addDraft.imageDataUrl || "" };
    ...
    // EDIT
    if (state.route.name === "edit"){
      if (!ensureAuth()) return;
      if (!state.editDraft || !state.editDraft.id){
        navTo({ name:"home", spiritId:null, prevTab:"home" });
        return;
      }

      app.innerHTML = addSpiritFormView("edit");

      document.getElementById("cancelForm")?.addEventListener("click", () => goBack());

      document.getElementById("removePhotoEdit")?.addEventListener("click", () => {
        state.editDraft.imageDataUrl = "";
        render();
      });

      document.getElementById("fileInputEdit")?.addEventListener("change", async (e) => {
        const f = e.target.files && e.target.files[0];
        if (!f) return;
        const dataUrl = await fileToDataUrl(f);
        state.editDraft.imageDataUrl = dataUrl;
        render();
      });

      document.getElementById("saveEdit")?.addEventListener("click", () => {
        const id = state.editDraft.id;
        const name = (document.getElementById("name")?.value || "").trim();
        ...
        state.spirits = state.spirits.map(s => s.id === id ? { ...s, name, brand, type, country, abv, notes, imageDataUrl: state.editDraft.imageDataUrl || "" } : s);
        persist();

        // back to detail
        navTo({ name:"detail", spiritId:id, prevTab: state.route.prevTab || "home" });
      });

      document.getElementById("deleteSpirit")?.addEventListener("click", () => {
        const id = state.editDraft.id;
        if (!confirm("Supprimer ce spiritueux ?")) return;

        state.spirits = state.spirits.filter(s => s.id !== id);
        delete state.cellar[id];
        state.reviews = state.reviews.filter(r => r.spiritId !== id);
        persist();

        navTo({ name: state.route.prevTab || "home", spiritId:null, prevTab: state.route.prevTab || "home" });
      });
    ...
    if (state.route.name === "addReview"){
      ...
      const update = () => { if (ratingText) ratingText.textContent = Number(rating.value).toFixed(1) + "/5"; };
      rating?.addEventListener("input", update);
      update();
      ...
      document.getElementById("saveReview")?.addEventListener("click", () => {
        ...
        persist();
        navTo({ name:"detail", spiritId: spirit.id, prevTab: state.route.prevTab || "home" });
      });
